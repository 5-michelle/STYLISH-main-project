function Cart(props) {
    ...
    function deleteItem(itemIndex) {
        const newCartItems = props.cartItems.filter((_, index) => index !== itemIndex);
        props.setCartItems(newCartItems);
        window.localStorage.setItem('cart', JSON.stringify(newCartItems));
        window.alert('已刪除商品');
    }

    const subtotal = props.cartItems.reduce((prev, item) => prev + item.price * item.qty, 0);
    ...
    return (
        <div className="cart">
            <div className="cart__header">
                <div className="cart__header-number">購物車({props.cartItems.length})</div>
                ...
                {props.cartItems.map((item, index) => (
                    <div className="cart__item" key={index}>
                        ...
                        <div className="cart__item-quantity">
                            ...
                            <select value={item.qty} className="cart__item-quantity-selector" onChange={(e) => changeQuantity(index, e.target.value)}>
                                {Array(item.stock)
                                    .fill()
                                    .map((_, index) => (
                        ...
                        <div className="cart__item-subtotal">
                            ...
                            <div className="cart__item-subtotal-content">NT.{item.qty * item.price}</div>
                        </div>
                        <div className="cart__delete-button" onClick={() => deleteItem(index)} />
                    </div>
                ))}
            </div>
            ...
            <div className="form">
                ...
                <div className="form__field">
                    ...
                    <input className="form__field-input" value={name} onChange={(e) => setName(e.target.value)} />
                </div>
                <div className="form__note">務必填寫完整收件人姓名，避免包裹無法順利簽收</div>
                <div className="form__field">
                    <div className="form__field-name">Email</div>
                    <input className="form__field-input" value={email} onChange={(e) => setEmail(e.target.value)} />
                </div>
                <div className="form__field">
                    <div className="form__field-name">手機</div>
                    <input className="form__field-input" value={phone} onChange={(e) => setPhone(e.target.value)} />
                </div>
                <div className="form__field">
                    <div className="form__field-name">地址</div>
                    <input className="form__field-input" value={address} onChange={(e) => setAddress(e.target.value)} />
                </div>
                <div className="form__field">
