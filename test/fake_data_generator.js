...
async function _createFakeUser(conn) {
    ...
    return await conn.query('INSERT INTO user (provider, role_id, email, password, name, picture, access_token, access_expired, login_at) VALUES ?', [
...
async function _createFakeProduct(conn) {
    return await conn.query('INSERT INTO product (id, category, title, description, price, texture, wash, place, note, story, main_image) VALUES ?', [
        products.map((x) => Object.values(x)),
    ]);
}
