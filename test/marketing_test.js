...
describe('marketing', () => {
    before(async () => {
        ...
        const res2 = await requester.post('/api/1.0/user/signin').send(user2);
        const data2 = res2.body.data;
        accessToken2 = data2.access_token;
    });

    it('get campaign data', async () => {
        const res = await requester.get('/api/1.0/marketing/campaigns');
    ...
    it('get hot data', async () => {
        const res = await requester.get('/api/1.0/marketing/hots');
    ...
    it('create hot data', async () => {
        await requester.post('/api/1.0/admin/hot').set('Authorization', `Bearer ${accessToken1}`).send({
            title: 'new hots',
            product_ids: '1,2,3',
    ...
    it('create hot data without login', async () => {
        const res = await requester.post('/api/1.0/admin/hot').send({
            title: 'new hots',
            product_ids: '1,2,3',
        });
    ...
    it('create hot data without admin role', async () => {
        const res = await requester.post('/api/1.0/admin/hot').set('Authorization', `Bearer ${accessToken2}`).send({
            title: 'new hots',
            product_ids: '1,2,3',
        });
